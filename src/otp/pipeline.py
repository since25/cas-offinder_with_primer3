...
def process_single_row(row, flank, designer, amplicon_min=150, amplicon_max=250, genome_profile=None):
    """
    Process primer design for a single off-target row.
    """
    genome = Genome(profile=genome_profile)
    chrom = row['chrom']
    pos0 = row['pos0']
...
def run_query(
    ...
    runner = CasOffinderRunner(fasta_dir=str(genome_profile.fasta_dir))
    ...
    if threads <= 1:
        for idx, row in df_off.iterrows():
            results.append(process_single_row(row, flank, designer, amplicon_min, amplicon_max, genome_profile))
    else:
...
def main():
    ...
    parser.add_argument("--gtf", type=str, default=None, help="Override GTF annotation file")
    ...
    genome_profile = get_genome_profile(args.genome)
    
    try:
        Genome(profile=genome_profile)
    ...
    for q in queries:
        df_res = run_query(
            ...
            cache_mgr, genome_profile, args.device
        )
        if not df_res.empty:
    ...
    # Annotate
    gtf_path = args.gtf or str(genome_profile.gtf_path)
    annotator = GTFAnnotator(gtf_path)
    final_df = annotator.annotate(final_df)
    
    # Export and Plot
    ExportManager.export_excel(final_df, str(Path(args.out) / "results.xlsx"), genome_profile=genome_profile)
